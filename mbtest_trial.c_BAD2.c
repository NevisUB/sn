...
//
    ...
    case 1:
      ...
      //scanf("%d",&nloop);
     nloop =1;
     printf("\t==> You can only have 1 loop\n");
...
//         buf_send[ij+1] = carray[2*ij+1]+ (carray[2*ij+2]<<16);
	...
	if ( count > 1) {
      ...
      printf("\t==> Not asking you to press 1 \n");
      usleep(500000);
      //scanf("%d",&ik);
...
//
      ...
      printf("\t==> Also not asking you to press 1\n");
      usleep(500000);
      //scanf("%d",&ik);
      //ik =1;
      if(ik ==1) {
...
//
       ...
       /* if (max__ < min__ || ( max__ == 0 && min__ == 0) )  { */
       /* 	 printf("\t==> Why is min greater than max OR you didn't give me input...  Error...\n");  */
       /* 	 return 3; */
       /* } */
...
//
      ...
      for (ik=0; ik< 64; ik++) {
       ...
       //vic //does this even do anything???
       //ijk = 0xfff;
       //ijk = 1;
      ...
      //
      // channel common threshold == 1
      
      /* buf_send[0]=(imod<<11)+(ichip<<8)+(mb_feb_tpc_sel_comthres)+((1 & 0xffff)<< 16); // channel common threshold */
      /* i = pcie_send(hDev3, 1, 1, px); */
      /* usleep(10); */
      ...
      ijk =10;
      buf_send[0]=(imod<<11)+(ichip<<8)+(mb_feb_tpc_load_thr_mean)+((ijk & 0xffff)<< 16); // load threshold mean...
      i = pcie_send(hDev3, 1, 1, px);
      usleep(10);
      ...
      //ijk = 0xfff; // max variance
      ijk = 100; // max variance
      buf_send[0]=(imod<<11)+(ichip<<8)+(mb_feb_tpc_load_thr_vari)+((ijk & 0xffff)<< 16); // load threshold variance
      i = pcie_send(hDev3, 1, 1, px);
      usleep(10);
      ...
      if(ihuff == 1) buf_send[0]=(imod<<11)+(ichip<<8)+mb_feb_b_nocomp+(0x0<<16);  // turn the compression
       ...
       timesize =4; //does this matter for SN?
       imod=imod_fem;
       ichip=3;
...
//
       ...
       //scanf("%d",&ik);
       printf("\t==> Not asking you to press 1 again again\n");
       usleep(500000);
...
//            if(idebug ==1) printf(" receive DMA complete %d \n", i);
	...
	//vic
	//DMA is complete !!!!
	printf("DMA is COMPLETE!: %d is = ", is);
...
//         ifr=1;
          printf(" buffer allocation 1\n");
          dwStatus = WDC_DMAContigBufLock(hDev4, &pbuf_rec1, dwOptions_rec, dwDMABufSize, &pDma_rec1);
          if (WD_STATUS_SUCCESS != dwStatus) {
           printf("Failed locking a rec 1 Contiguous DMA buffer. Error 0x%lx - %s\n", dwStatus, Stat2Str(dwStatus));
           printf("enter 1 to continue \n");
           scanf("%d",&is);
...
//         ifr=1;
          printf(" buffer allocation 2\n");
          dwStatus = WDC_DMAContigBufLock(hDev4, &pbuf_rec2, dwOptions_rec, dwDMABufSize, &pDma_rec2);
          if (WD_STATUS_SUCCESS != dwStatus) {
           printf("Failed locking a rec 2 Contiguous DMA buffer. Error 0x%lx - %s\n", dwStatus, Stat2Str(dwStatus));
           printf("enter 1 to continue \n");
           scanf("%d",&is);
...
/***    check to see if DMA is done or not **/
         ...
         for (is=0; is<6000; is++) {;
          ...
          u64Data =0;
          dwOffset = cs_dma_cntrl;
          WDC_ReadAddr32(hDev4, dwAddrSpace, dwOffset, &u32Data);
          if(idebug ==1) printf(" receive DMA status word %d %X \n", is, u32Data);
          if((u32Data & dma_in_progress) == 0) {
           idone =1;
//           if(idebug ==1) printf(" receive DMA complete %d \n", i);
...
//
          dwAddrSpace =cs_bar;
          dwOffset = cs_dma_by_cnt;
          WDC_ReadAddr64(hDev4, dwAddrSpace, dwOffset, &u64Data);
...
//     scanf("%d",&nloop);
     ...
     scanf("%d", &itpc_adc);
     if(itpc_adc !=1) {
       printf(" type 1 for kazu fake data \n");
       scanf("%d", &ifake_kazu);
       if(ifake_kazu ==1) {
        printf(" enter compression factor \n");
        scanf("%d",&icom_factor);
     ...
     if(itrig_c == 1) {
      ...
      if(itrig_pulse == 2) {
        printf(" enter frame trigger, sample number \n");
        scanf("%d", &frame_trig_sample);
        printf(" enter frame trigger, div number \n");
        scanf("%d", &frame_trig_div); 
      }   
     }
...
//         else ijk=ibase;
         ...
         if(ifake_kazu ==1) ijk = send_array[is*256+ik];
         k = 0x8000+ ijk;        // make sure bit 15-12 is clear for the data
 //        printf("k = %x\n", k);
 //        scanf("%d", &ijk);
         buf_send[0]=(imod<<11)+(ichip<<8)+(mb_feb_test_ram_data)+((k & 0xffff)<<16); // load test data
...
//
         ...
         imod = imod_trig;
         buf_send[0]=(imod<<11)+(mb_trig_frame_trig_frm)+( frame_trig_sample <<16); // trigger out
         i=1;
         k=1;
         i = pcie_send(hDev, i, k, px);

         imod = imod_trig;
         buf_send[0]=(imod<<11)+(mb_trig_frame_trig_div)+( frame_trig_div <<16); // trigger out
         i=1;
         k=1;
         i = pcie_send(hDev, i, k, px);
...
//       nword = nword+1;
       ...
       if(iprint == 1) {
        for (i=0; i< nword; i++) {
         ...
         if(((i+1)%8) ==0 ) printf("\n");
         checksum = checksum+ (read_array[i] & 0xffff);
         checksum = checksum+ ((read_array[i] >>16)& 0xffff);
        }
        printf(" \n checksum= %x \n", checksum);
       }
       if(iwrite == 1) {
