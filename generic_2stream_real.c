...
static char *location__, *xmitFPGAFileName__, *femFPGAFileName__, *baselinesFileName__, *outTrigFileName__, *outSnovaFileName__;
static int xmitMod__, femMod__, amplitudeThresh__, fixBaseline__, baseMeanTolerance__, baseVarianceTolerance__, presamples__, postsamples__, bipolar__;
...
int main(int argc, char **argv)
{
  ...
  if( argc != 16 ){
    printf("Run it using one of the bash scripts RunAt*.sh \n");
    printf("Exit...\n");
    exit(-1);
  } 

  location__ = argv[1];
  xmitFPGAFileName__ = argv[2];
  xmitMod__ = atoi(argv[3]);
  femFPGAFileName__ = argv[4];
  femMod__ = atoi(argv[5]);
  outTrigFileName__ = argv[6];
  outSnovaFileName__ = argv[7];
  amplitudeThresh__ = atoi(argv[8]);
  fixBaseline__ = atoi(argv[9]);
  baselinesFileName__ = argv[10];
  baseMeanTolerance__ = atoi(argv[11]);
  baseVarianceTolerance__ = atoi(argv[12]);
  presamples__ = atoi(argv[13]);
  postsamples__ = atoi(argv[14]);
  bipolar__ = atoi(argv[15]);

  printf( "Location=%s\n", location__ );
  printf( "XMIT_FPGACode=%s\n", xmitFPGAFileName__ );
  printf( "XMIT_Module=%i\n", xmitMod__ );
  printf( "FEM_FPGACode=%s\n", femFPGAFileName__ );
  printf( "FEM_Module=%i\n", femMod__ );
  printf( "OutputTriggerFile=%s\n", outTrigFileName__ );
  printf( "OutputSupernovaFile=%s\n", outSnovaFileName__ );
  printf( "AmplitudeThreshold=%i\n", amplitudeThresh__ );
  printf( "FixBaseline=%i\n", fixBaseline__ );
  printf( "BaselinesFile=%s\n", baselinesFileName__ );
  printf( "BaselineMeanTolerance=%i\n", baseMeanTolerance__ );
  printf( "BaselineVarianceTolerance=%i\n", baseVarianceTolerance__ );
  printf( "Presamples=%i\n", presamples__ );
  printf( "Postsamples=%i\n", postsamples__ );
  printf( "Bipolar=%i\n", bipolar__ );
  ...
  */
  ...
  case 5:
    ...
    for (j=0; j<nloop; j++) {
      ...
      inpf = fopen(xmitFPGAFileName__, "r");
      imod=imod_xmit;
...
#define  dma_buffer_size        10000000
  ...
  inpf = fopen(femFPGAFileName__, "r");
