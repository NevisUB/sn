...
static void MenuMain(WDC_DEVICE_HANDLE *phDev, WDC_DEVICE_HANDLE *phDev1, WDC_DEVICE_HANDLE *phDev2, 
		     WDC_DEVICE_HANDLE *phDev3);

/* -----------------------------------------------
   Device find, open and close
   ----------------------------------------------- */
static WDC_DEVICE_HANDLE DeviceFindAndOpen(DWORD dwVendorId, DWORD dwDeviceId, int choice);
static BOOL DeviceFind(DWORD dwVendorId, DWORD dwDeviceId, WD_PCI_SLOT *pSlot, int choice);
...
int trig_module_read(WDC_DEVICE_HANDLE hDev);
...
static DWORD dwDMABufSize_pmt = 1000000;
//static DWORD dwDMABufSize_pmt = 1000;
static DWORD dwDMABufSize_tpc = 1000000;
...
#define  jbuf_ev_size_pmt 1000000
#define  jbuf_ev_size_tpc 1000000
...
static int buffer_ev_n[jbuf_ev_size_pmt], buffer_ev_s[jbuf_ev_size_pmt];
static int buffer_ev_n_tpc[jbuf_ev_size_tpc], buffer_ev_s_tpc[jbuf_ev_size_tpc];
...
int main(void)
{
  ...
  if (PCIE_DEFAULT_VENDOR_ID)
    hDev = DeviceFindAndOpen(PCIE_DEFAULT_VENDOR_ID, PCIE_DEFAULT_DEVICE_ID, 1);
  if (PCIE_DEFAULT_VENDOR_ID)
    hDev1 = DeviceFindAndOpen(PCIE_DEFAULT_VENDOR_ID, PCIE_DEFAULT_DEVICE_ID, 4);
  if (PCIE_DEFAULT_VENDOR_ID)
  ...
  */
  ...
  MenuMain(&hDev, &hDev1, &hDev2, &hDev3);
  ...
  do
    {
	  ...
	  *phDev = DeviceFindAndOpen(0, 0, 0);
	  ...
	  MenuMBtest(*phDev, *phDev1, *phDev2, *phDev3, 0, 0);
	  option = MENU_MAIN_EXIT;
	  break;
...
static WDC_DEVICE_HANDLE DeviceFindAndOpen(DWORD dwVendorId, DWORD dwDeviceId, int choice)
{
  ...
  if (!DeviceFind(dwVendorId, dwDeviceId, &slot, choice))
    return NULL;
...
static BOOL DeviceFind(DWORD dwVendorId, DWORD dwDeviceId, WD_PCI_SLOT *pSlot, int choice)
{
  ...
  *pSlot = scanResult.deviceSlot[choice - 1];
...
// hDev  = PMT crate controller + Trigger readout
// hDev1 = PMT neutrino
// hDev2 = PMT SN
// hDev3 = TPC crate controller
// hDev4 = TPC neutrino
// hDev5 = TPC SN

// Following comment is if you use prompt option in the order: 1 4 2 3
static void MenuMBtest(WDC_DEVICE_HANDLE hDev,  // Valid PCIE for PMT crate + Trigger
		       WDC_DEVICE_HANDLE hDev1, // Valid PCIE for PMT Neutrino xmit
		       WDC_DEVICE_HANDLE hDev3, // Valid PCIE for TPC crate controller
		       WDC_DEVICE_HANDLE hDev4, // Valid PCIE for TPC Neutrino xmit
		       WDC_DEVICE_HANDLE hDev2, // Invalid PCIE (void)
		       WDC_DEVICE_HANDLE hDev5) // Invalid PCIE (void)
{
...
#define  mb_trig_fakegate_select   80
#define  mb_trig_scaler1_select    81
#define  mb_trig_scaler2_select    82
...
#define  dma_buffer_size        40000000
  ...
  */
  ...
  case 7:
    ...
    //     scanf("%d",&nevent);
    //
    printf(" type 1 to use trigger module as pulser, 2 for frame trigger\n");
    scanf("%d", &itrig_pulse);
    if (itrig_pulse == 1) {
    ...
    ineu = 0;
    isuper = 0;
    printf("[0,1,2,3,4] for [pmt nu, pmt+tpc nu, pmt nu+sn, tpc nu+sn, tpc nu] \n");
    int readmode=99;
    scanf("%d", &readmode);
    if(readmode == 99)
      break;
      ...
      ipmt_read = 1;
      itpc_read = 0;
      // Assign hDev2 (pmt SN) = hDev4 (tpc NU)
      hDev2 = hDev4;
    }
    else if(readmode == 3) {
      ineu = 1;
      ...
      ipmt_read = 0;
      itpc_read = 1;
      // Assign hDev5 (tpc SN) = hDev1 (pmt NU)
      hDev5 = hDev1;
    }
    else if(readmode == 4) {
      ineu = 1;
    ...
    */
    ...
    timesize = 500; // works
    //timesize = 2000; // works
    //timesize = 3196; //doesn't work
    //timesize = 3198; //doesn't work
    //timesize = 3200; 
    //itrig_delay = 10;
    ...
    //     timesize =4;
    //dwDMABufSize = 2000000;
    printf(" frame size (16 MHz) = %d, timesize = %d\n",iframe_length+1, timesize);
    printf(" PMT DMA buffer size = %d\n", dwDMABufSize_pmt);
    printf(" TPC DMA buffer size = %d\n", dwDMABufSize_tpc);
	...
	dwOffset = 0x18;
	dwStatus = WDC_WriteAddr32(hDev2, dwAddrSpace, dwOffset, u32Data);
	if (WD_STATUS_SUCCESS != dwStatus) {
	  printf("Failed initializing xmit transmitter. Error 0x%lx - %s\n", dwStatus, Stat2Str(dwStatus));
	  break;
	}
	dwAddrSpace =2;
	u32Data = 0x20000000;    // initial transmitter, no hold
    ...
    //printf("Trigger Check point: after doing mb_trig_enable 0x0\n");
    //scanf("%d",&ik);
    ...
    //buf_send[0]=(imod<<11)+(mb_trig_deadtime_size)+(0x32<<16); //set trigger module deadtime size
    //buf_send[0]=(imod<<11)+(mb_trig_deadtime_size)+(0xc8<<16); //set trigger module deadtime size
    buf_send[0]=(imod<<11)+(mb_trig_deadtime_size)+(0x64<<16); //set trigger module deadtime size
    //buf_send[0]=(imod<<11)+(mb_trig_deadtime_size)+(0x32<<16); //set trigger module deadtime size
    //buf_send[0]=(imod<<11)+(mb_trig_deadtime_size)+(0x320<<16); //set trigger module deadtime size
    i=1;
    k=1;
    i = pcie_send(hDev, i, k, px);

    //printf("Trigger Check point: after doing mb_deadtime_size 0x320\n");
    //scanf("%d",&ik);
    ...
    //printf("Trigger Check point: after doing mb_trig_output_select 0x1 or 0x2\n");
    //scanf("%d",&ik);      
      
    //
    //
    //
    //   starting boot process for the PMT
    //
    //
    //    boot up xmit module 1st
    //
    if(ipmt_read == 1) {
      printf(" booting PMT XMIT\n");
      //scanf("%d",&ik);
      nword =1 ;
      ...
      printf(" ***** Slow Ctrl Monitoring: PMT-XMIT Readout Status Info *****\n");
      ...
      inpf = fopen("../..//xmit_fpga_link_header","r");
      imod=imod_xmit;
	...
	counta++;
	if((count%(nsend*2)) == 0) {
	  if(dummy1%100==0) printf(".");
	  if(dummy1%1000==0) printf("\n");
	  //        printf(" loop = %d\n",dummy1);
	...
	i = pcie_send(hDev, i, nword, px);
      ...
      printf("\033[93m BEFORE for loop to set adc \033[00m\n");
      check_status(hDev, 0, imod_trig, imod_xmit+1, imod_st, imod_xmit_tpc+1, imod_st_tpc, imod_xmit, imod_xmit_tpc);
	...
	ik=pmt_adc_setup(hDev,imod_fem,iframe);
      ...
      usleep(1000);
      printf("\033[93m AFTER for loop to set adc \033[00m\n");
      check_status(hDev, 0, imod_trig, imod_xmit+1, imod_st, imod_xmit_tpc+1, imod_st_tpc, imod_xmit, imod_xmit_tpc);
      printf("finish booting PMT fems....\n");
      //scanf("%d",&ik);
      //
    ...
    //
    //
    //   starting boot process for the TPC
    //
    //
    //    boot up xmit module 1st
    //
    if(itpc_read == 1) {
      printf(" booting TPC XMIT\n");
      //scanf("%d",&ik);
      inpf = fopen("../../xmit_fpga_link_header","r");
      imod=imod_xmit_tpc;
      ichip=mb_xmit_conf_add;
	...
	ik=tpc_adc_setup(hDev3, imod_fem, iframe, itpc_adc, ihuff, icom_factor, timesize);
      ...
      printf(" booting TPC FEM modules done\n");
      //scanf("%d",&ik);
      //
      //    both FEM and XMIT bootted.
      //
    ...
    //
    imod=imod_trig;
    buf_send[0]=(imod<<11)+(mb_trig_mask0)+((0x0)<<16); //set mask1[3] on.
    //buf_send[0]=(imod<<11)+(mb_trig_mask0)+((0xffff)<<16); //set mask1[3] on.
    i=1;
    k=1;
    i = pcie_send(hDev, i, k, px);
    //
    ...
    //
    imod=imod_trig;
    buf_send[0]=(imod<<11)+(mb_trig_mask1)+((0x0)<<16); //set mask1[3] on.
    //buf_send[0]=(imod<<11)+(mb_trig_mask1)+((0xffff)<<16); //set mask1[3] on.
    i=1;
    k=1;
    i = pcie_send(hDev, i, k, px);
    //
    //    set mask8 bit 3 high
    //
    imod=imod_trig;
    //buf_send[0]=(imod<<11)+(mb_trig_mask8)+((0x4 + 0x10)<<16); //set mask1[3] on.
    //buf_send[0]=(imod<<11)+(mb_trig_mask8)+((0x8 + 0x20)<<16); //set mask1[3] on.
    buf_send[0]=(imod<<11)+(mb_trig_mask8)+((0x2 + 0x8 + 0x20 + 0x10)<<16); //set mask1[3] on.
    //buf_send[0]=(imod<<11)+(mb_trig_mask8)+((0x8)<<16); // PCTRIG ON
    //buf_send[0]=(imod<<11)+(mb_trig_mask8)+((0x2)<<16); //EXT-TRIG ON
    //buf_send[0]=(imod<<11)+(mb_trig_mask8)+((0x2 + 0x8)<<16); //EXT-TRIG + PCTRIG ON
    i=1;
    k=1;
    i = pcie_send(hDev, i, k, px);
    //
    ...
    //
    imod=imod_trig;
    buf_send[0]=(imod<<11)+(mb_trig_prescale0)+(0x0<<16); //set prescale1 0
    i=1;
    k=1;
    i = pcie_send(hDev, i, k, px);
    //
    ...
    buf_send[0]=(imod<<11)+mb_trig_g1_allow_min+((0x1)<<16);
    i=1;
    k=1;
    i = pcie_send(hDev, i, k, px);
    usleep(1000);
    imod =imod_trig;  /* trigger module */
    buf_send[0]=(imod<<11)+mb_trig_g1_allow_max+((0x1)<<16);
    i=1;
    k=1;
    i = pcie_send(hDev, i, k, px);
    usleep(1000);
    buf_send[0]=(imod<<11)+mb_trig_g1_width+((0x20)<<16);
    //buf_send[0]=(imod<<11)+mb_trig_g1_width+((0x1)<<16);
    i=1;
    k=1;
    i = pcie_send(hDev, i, k, px);
    usleep(1000);
    buf_send[0]=(imod<<11)+mb_trig_delay1_size+((0x10)<<16);
    //buf_send[0]=(imod<<11)+mb_trig_delay1_size+((0x30)<<16);
    i=1;
    k=1;
    i = pcie_send(hDev, i, k, px);
    usleep(1000);
    ...
    imod =imod_trig;  /* trigger module */
    buf_send[0]=(imod<<11)+mb_trig_g2_allow_min+((0x1)<<16);
    i=1;
    k=1;
    i = pcie_send(hDev, i, k, px);
    usleep(1000);
    imod =imod_trig;  /* trigger module */
    buf_send[0]=(imod<<11)+mb_trig_g2_allow_max+((0x20)<<16);
    i=1;
    k=1;
    i = pcie_send(hDev, i, k, px);
    usleep(1000);
    buf_send[0]=(imod<<11)+mb_trig_g2_width+((0x20)<<16);
    //buf_send[0]=(imod<<11)+mb_trig_g2_width+((0x1)<<16);
    i=1;
    k=1;
    i = pcie_send(hDev, i, k, px);
    usleep(1000);
    buf_send[0]=(imod<<11)+mb_trig_delay2_size+((0x10)<<16);
    //buf_send[0]=(imod<<11)+mb_trig_delay2_size+((0x30)<<16);
    i=1;
    k=1;
    i = pcie_send(hDev, i, k, px);
    usleep(1000);

    // fake beamgate select 0x1 = use ext as fake beamgate
    buf_send[0]=(imod<<11)+mb_trig_fakegate_select+((0x0)<<16);
    i=1;
    k=1;
    i = pcie_send(hDev, i, k, px);
    usleep(1000);
    ...
    // scaler1 select
    buf_send[0]=(imod<<11)+mb_trig_scaler1_select+((0xffff)<<16);
    i=1;
    k=1;
    i = pcie_send(hDev, i, k, px);
    usleep(1000);
    ...
    // scaler2 select 
    buf_send[0]=(imod<<11)+mb_trig_scaler2_select+((0xf6)<<16);
    i=1;
    k=1;
    i = pcie_send(hDev, i, k, px);
    usleep(1000);
    ...
    */
    ...
    if(ipmt_read == 1) {
      ...
      printf("\033[93m BEFORE pmt_link_setup \033[00m\n");
      check_status(hDev, 0, imod_trig, imod_xmit+1, imod_st, imod_xmit_tpc+1, imod_st_tpc, imod_xmit, imod_xmit_tpc);
      usleep(1000);
      ...
      usleep(1000);
      printf("\033[93m AFTER pmt_link_setup \033[00m\n");
      check_status(hDev, 0, imod_trig, imod_xmit+1, imod_st, imod_xmit_tpc+1, imod_st_tpc, imod_xmit, imod_xmit_tpc);
      //
      //
      //
    ...
    if(itpc_read == 1) {
      ...
      ik = pmt_link_setup(hDev3,imod_xmit_tpc,imod_st_tpc,0);      // no supernova
      //
      //
      //
      //
    ...
    //
    //       allocate Neutrino Buffer 1 for PMT
    //
    if(ipmt_read == 1) {
      printf(" buffer allocation --- Neutrino 1\n");
      dwStatus = WDC_DMAContigBufLock(hDev1, &pbuf_rec_n1, dwOptions_rec, dwDMABufSize_pmt, &pDma_rec_n1);
      if (WD_STATUS_SUCCESS != dwStatus) {
	...
	printf(" buffer allocation 1 higher address = %x\n", u32Data);
      ...
      dwStatus = WDC_DMAContigBufLock(hDev1, &pbuf_rec_n2, dwOptions_rec, dwDMABufSize_pmt, &pDma_rec_n2);
      if (WD_STATUS_SUCCESS != dwStatus) {
	...
	dwStatus = WDC_DMAContigBufLock(hDev2, &pbuf_rec_s1, dwOptions_rec, dwDMABufSize_pmt, &pDma_rec_s1);
	if (WD_STATUS_SUCCESS != dwStatus) {
	...
	dwStatus = WDC_DMAContigBufLock(hDev2, &pbuf_rec_s2, dwOptions_rec, dwDMABufSize_pmt, &pDma_rec_s2);
	if (WD_STATUS_SUCCESS != dwStatus) {
	...
	dwStatus = WDC_DMAContigBufLock(hDev2, &pbuf_rec_s3, dwOptions_rec, dwDMABufSize_pmt, &pDma_rec_s3);
	if (WD_STATUS_SUCCESS != dwStatus) {
	...
	dwStatus = WDC_DMAContigBufLock(hDev2, &pbuf_rec_s4, dwOptions_rec, dwDMABufSize_pmt, &pDma_rec_s4);
	if (WD_STATUS_SUCCESS != dwStatus) {
    ...
    if(itpc_read == 1) {
      ...
      dwStatus = WDC_DMAContigBufLock(hDev4, &pbuf_rec_n1_tpc, dwOptions_rec, dwDMABufSize_tpc, &pDma_rec_n1_tpc);
      if (WD_STATUS_SUCCESS != dwStatus) {
	...
	printf(" buffer allocation 1 higher address = %x\n", u32Data);
      ...
      dwStatus = WDC_DMAContigBufLock(hDev4, &pbuf_rec_n2_tpc, dwOptions_rec, dwDMABufSize_tpc, &pDma_rec_n2_tpc);
      if (WD_STATUS_SUCCESS != dwStatus) {
	...
	dwStatus = WDC_DMAContigBufLock(hDev5, &pbuf_rec_s1_tpc, dwOptions_rec, dwDMABufSize_tpc, &pDma_rec_s1_tpc);
	if (WD_STATUS_SUCCESS != dwStatus) {
	...
	dwStatus = WDC_DMAContigBufLock(hDev5, &pbuf_rec_s2_tpc, dwOptions_rec, dwDMABufSize_tpc, &pDma_rec_s2_tpc);
	if (WD_STATUS_SUCCESS != dwStatus) {
	  printf("Failed locking a SuperNova Receive Contiguous DMA buffer 2. Error 0x%lx - %s\n", dwStatus, Stat2Str(dwStatus));
	  //printf("enter 1 to continue \n");
	  //scanf("%d",&is);
	...
	dwStatus = WDC_DMAContigBufLock(hDev5, &pbuf_rec_s3_tpc, dwOptions_rec, dwDMABufSize_tpc, &pDma_rec_s3_tpc);
	if (WD_STATUS_SUCCESS != dwStatus) {
	  printf("Failed locking a SuperNova Receive Contiguous DMA buffer 3. Error 0x%lx - %s\n", dwStatus, Stat2Str(dwStatus));
	  //printf("enter 1 to continue \n");
	  //scanf("%d",&is);
	...
	dwStatus = WDC_DMAContigBufLock(hDev5, &pbuf_rec_s4_tpc, dwOptions_rec, dwDMABufSize_tpc, &pDma_rec_s4_tpc);
	if (WD_STATUS_SUCCESS != dwStatus) {
	  printf("Failed locking a SuperNova Receive Contiguous DMA buffer 4. Error 0x%lx - %s\n", dwStatus, Stat2Str(dwStatus));
	  //printf("enter 1 to continue \n");
	  //scanf("%d",&is);
    ...
    //
    //    set up the trigger module readback
    //
    ...
    printf("\n \033[93mTrigger Parameter Check \033[00m\n");
    // TRIGGER STATUS
    nword = 8;
    i = pcie_rec(hDev,0,1,nword,iprint,py);     // init the receiver
    imod=imod_trig;
    ...
    printf("Frame size    : %d\n",((read_array[0] >>16) & 0xffff));
    printf("Deadtime size : %d\n",(read_array[1] & 0xff));
    printf("Prescale 0    : 0x%x\n",((read_array[1]>>24) & 0xf));
    printf("Prescale 1    : 0x%x\n",((read_array[1]>>16) & 0xff));
    printf("Prescale 2    : 0x%x\n",((read_array[2]>>8) & 0xf));
    printf("Prescale 3    : 0x%x\n",(read_array[2] & 0xff));
    printf("Prescale 4    : 0x%x\n",((read_array[2]>>24) & 0xf));
    printf("Prescale 5    : 0x%x\n",((read_array[2]>>16) & 0xff));
    printf("Prescale 6    : 0x%x\n",((read_array[3]>>8) & 0xf));
    printf("Prescale 7    : 0x%x\n",(read_array[3] & 0xff));
    printf("Prescale 8    : 0x%x\n",((read_array[3]>>24) & 0xf));
    printf("Mask 0        : 0x%x\n",((read_array[4]>>8) & 0xf));
    printf("Mask 1        : 0x%x\n",(read_array[4] & 0xff));
    printf("Mask 2        : 0x%x\n",((read_array[4]>>24) & 0xf));
    printf("Mask 3        : 0x%x\n",((read_array[4]>>16) & 0xff));
    printf("Mask 4        : 0x%x\n",((read_array[5]>>8) & 0xf));
    printf("Mask 5        : 0x%x\n",(read_array[5] & 0xff));
    printf("Mask 6        : 0x%x\n",((read_array[5]>>24) & 0xf));
    printf("Mask 7        : 0x%x\n",((read_array[5]>>16) & 0xff));
    printf("Mask 8        : 0x%x\n",((read_array[6]>>8) & 0xf));
    printf("Delay1        : %d\n",((read_array[6]>>16) & 0xffff));
    printf("Delay2        : %d\n",(read_array[7] & 0xffff));

    printf("type 1 to set trigger-on \n");
    //scanf("%d",&i);
    //printf("\n\ntype 1 to issue trigger run ON \n");
    //scanf("%d",&i);    
    printf("\n \033[93mSTATUS CHECK\033[00m ... BEFORE trigger run ON\n");
    if(itpc_read){
      if(check_status(hDev, hDev3, imod_trig, imod_xmit+1, imod_st, imod_xmit_tpc+1, imod_st_tpc, imod_xmit, imod_xmit_tpc) )
	break;
    }
    else if(check_status(hDev, 0, imod_trig, imod_xmit+1, imod_st, imod_xmit_tpc+1, imod_st_tpc, imod_xmit, imod_xmit_tpc))
      break;
    imod=imod_trig;
    ...
    /*
    printf("\n \033[93mSTATUS CHECK\033[00m ... AFTER trigger run ON + 6.4ms BEFORE trigger enable \n\n");
    if(itpc_read && check_status(hDev, hDev3, imod_trig, imod_xmit+1, imod_st, imod_xmit_tpc+1, imod_st_tpc, imod_xmit, imod_xmit_tpc) )
      break;
    else if(check_status(hDev, 0, imod_trig, imod_xmit+1, imod_st, imod_xmit_tpc+1, imod_st_tpc, imod_xmit, imod_xmit_tpc))
      break;
    */
    //
    //  set trigger module to enable triggers
    //
    printf("type 1 to enable trigger \n");
    //scanf("%d",&i);
    imod=imod_trig;
    buf_send[0]=(imod<<11)+(mb_trig_enable)+(0x1<<16); //enable all triggers
    i=1;
    ...
    /*
    printf("\n\033[93m STATUS CHECK \033[00m... AFTER trigger enable \n\n");
    if(itpc_read && check_status(hDev, hDev3, imod_trig, imod_xmit+1, imod_st, imod_xmit_tpc+1, imod_st_tpc, imod_xmit, imod_xmit_tpc) )
      break;
    else if(check_status(hDev, 0, imod_trig, imod_xmit+1, imod_st, imod_xmit_tpc+1, imod_st_tpc, imod_xmit, imod_xmit_tpc))
      break;
    */    
    //printf("type 1 to move onto while loop... \n");
    //scanf("%d",&i);
    ...
    */
    ...
    int new_trigger=0;
    int trigger_count=0;
    int max_trigger_count=35000;
    int check_status_every_trigger=0;
    int respect_status=0;

    while (1) {

      if(kazu_ctr%1000==0 || (check_status_every_trigger && new_trigger)) {
	if(itpc_read) {
	  if(check_status(hDev, hDev3, imod_trig, imod_xmit+1, imod_st, imod_xmit_tpc+1, imod_st_tpc, imod_xmit, imod_xmit_tpc) && respect_status )
	    break;
	}
	else if(check_status(hDev, 0, imod_trig, imod_xmit+1, imod_st, imod_xmit_tpc+1, imod_st_tpc, imod_xmit, imod_xmit_tpc) && respect_status)
	  break;

	/*
	if( (kazu_ctr/1000)%2 ) {
	  /// Trigger off when (2n+1)*1000
	  imod=imod_trig;
	  buf_send[0]=(imod<<11)+(mb_trig_enable)+(0x0<<16); //enable all triggers
	  i=1;
	  k=1;
	  i = pcie_send(hDev, i, k, px);
	  usleep(6400);          // 6.4ms
	  /// Run off when (2n+1)*1000
	  imod=imod_trig;
	  buf_send[0]=(imod<<11)+(mb_trig_run)+(0x0<<16); //enable all triggers
	  i=1;
	  k=1;
	  i = pcie_send(hDev, i, k, px);
	  usleep(6400);          // 6.4ms
	}else{
	  /// Trigger on when 2n*1000
	  imod=imod_trig;
	  buf_send[0]=(imod<<11)+(mb_trig_enable)+(0x1<<16); //enable all triggers
	  i=1;
	  k=1;
	  i = pcie_send(hDev, i, k, px);
	  usleep(6400);          // 6.4ms
	  /// Run on when (2n+1)*1000
	  imod=imod_trig;
	  buf_send[0]=(imod<<11)+(mb_trig_run)+(0x1<<16); //enable all triggers
	  i=1;
	  k=1;
	  i = pcie_send(hDev, i, k, px);
	  usleep(6400);          // 6.4ms
	...
	*/
      ...
      trigger_count += new_trigger;
      if(trigger_count>= max_trigger_count) break;
      new_trigger = 0;
	...
	if(key == 'q') {
	  ...
	  printf("\033[93m[ WARNING ]\033[00m Ignoring user input (only 'q' acceptable to quit)!\n");
      }
      ...
      //
      //
      if(itrig_pulse == 1 && kazu_ctr%250==0) {
	imod = imod_trig;
	buf_send[0]=(imod<<11)+(mb_trig_p1_delay)+((p1_delay)<<16); // set up p1 delay;
	i=1;
	k=1;
	...
	//
	usleep(1000);
	printf("\033[1;34;40m[ Sending Trigger Pulse ]\033[00m\n");
      }
	...
	if(ipmt_read == 1) {
	  ...
	  i = pcie_rec(hDev,0,2,nword,iprint,py);     // read out 2 32 bits words
	  //
	  //if(ith_fr != 1 || kazu_ctr%1000==0) {
	  if(ith_fr != 1) {
	    printf(" ***** Slow Ctrl Monitoring: PMT-XMIT Counter Info *****\n");
	...
	if(itpc_read == 1) {
	  ...
	  //if(ith_fr != 1 || kazu_ctr%1000==0) {
	  if(ith_fr != 1) {
	    printf(" ***** Slow Ctrl Monitoring: TPC-XMIT Counter Info *****\n");
	    printf("    buffer status word = %x, %x, %x, %x, %x\n", read_array[0], read_array[1],
		   read_array[2], read_array[3], read_array[4]);
	    printf("    crate %d, header %x \n",(read_array[0] & 0x1f) ,((read_array[0] >> 8) & 0xff));
	    printf("    frame count %d \n", read_array[1]);
      ...
      //
      //      new_trigger += trig_module_read(hDev);
      //
      //
      //
      if((itrig_m == 1) & (itrig_m_d != 1)) {
	...
	for (imod_fem = imod_last; imod_fem< (imod_st+1); imod_fem++) {
	  if(imonitor == 1) {
	    ...
	    //if(ith_fr != 1 || kazu_ctr%1000 == 0) {
	    if(ith_fr != 1) {
	      printf(" ***** Slow Ctrl Monitoring: PMT-FEM NU-Buffer Status Info *****\n");
	  ...
	  if(imonitor == 1) {
	    ...
	    //if(ith_fr != 1 || kazu_ctr%1000 == 0) {
	    if(ith_fr != 1) {
	      printf(" ***** Slow Ctrl Monitoring: PMT-FEM SN-Buffer Status Info *****\n");
	...
	/*
	  printf(" ***** Slow Ctrl Monitoring: Trigger Status (ctr) Info *****\n");
	  printf("    Frame          : %d\n", (read_array[1] & 0xffffff));
	  printf("    Sample (2MHz)  : %d\n", (read_array[2] & 0xfff));
	  printf("    Sample (16MHz) : %d\n", (read_array[2] & 0xfff)*8 + ((read_array[2]>>16) & 0x7));
	  printf("    Trigger ctr    : %d\n", (read_array[3] & 0xffffff));
	  for(i=4; i<12; i++)
	  printf("    PMT bit %d ctr  : %d\n", (i-4), read_array[i]);
	  printf("\n");
	  for (i=0; i< 12; i++) {
	  printf(" i= %d, data = %x \n", i, read_array[i]);
	  }
	*/
	...
	/*
	  printf(" ***** Slow Ctrl Monitoring: PMT FEM Counter (ctr) Info ***** \n");
	  printf("    Header = %x\n",read_array[0]);
	  printf("    Time Counter (?): %d",read_array[1]);
	  printf("    Channel-wise discriminator counters:\n");
	  for(i=2; i<41; i++) {
	  printf("    Ch. %02d = %-6d",i-2,read_array[i]);
	  if((i-1)%4 == 0) printf("\n");
	  }
	  printf("\n");
	*/
	...
	*/
    ...
    kazu_ctr=5;
    while(kazu_ctr>0) {
      printf("Trigger disabled. Sleep %d seconds and turn off run, and exit...\n",kazu_ctr);
      kazu_ctr = kazu_ctr-1;
      usleep(1000000);
    }
    imod=imod_trig;
    buf_send[0]=(imod<<11)+(mb_trig_run)+((0x0)<<16); //set up run
    i=1;
    k=1;
    i = pcie_send(hDev, i, k, px);
    printf("Trigger run off...\n");

    close(fd_trig_pt);
    printf("PMT file closed...\n");

    close(fd_trig_pt_tpc);
    printf("TPC file closed...\n");

    close(fd_trig_pt_m);
    printf("Trigger file closed...\n");
...
#define mb_trig_rd_status 32
  ...
  if(hDevPMT) {
    printf(" *****\033[95m Slow Ctrl Monitoring:\033[93m PMT-FEM Status Info \033[00m*****\n");
    printf(" module: b-err1: b-err2: pll   : snmem : bmmem : header: data  : r-pll : l-pll : r-dpa : l-dpa : r-cmd : l-cmd : r-done: l-done\n");
    for(imod_fem_pmt = imod_fem_pmt_start; imod_fem_pmt <= imod_fem_pmt_end; imod_fem_pmt++) {
      // PMT FEM STATUS
      nword =2 ;
      i = pcie_rec(hDevPMT,0,1,nword,iprint,py);     // init the receiver
      imod=imod_fem_pmt;
      ichip=3;
      buf_send[0]=(imod<<11)+(ichip<<8)+20+(0x0<<16);  // read out status
      i=1;
      k=1;
      i = pcie_send(hDevPMT, i, k, px);
      py = &read_array;
      i = pcie_rec(hDevPMT,0,2,nword,iprint,py);     // read out 2 32 bits words

      printf("\033[91m FEM status words... \033[00m"); 
      printf("\n\033[93m");
      printf("0x%x 0x%x\n",read_array[0],read_array[1]);
      printf("\033[00m\n");
    ...
    // PMT FEM COUNTER
    printf(" *****\033[95m Slow Ctrl Monitoring: \033[93mPMT FEM Counter (ctr) Info \033[00m***** \n");
    for(imod_fem_pmt = imod_fem_pmt_start; imod_fem_pmt <= imod_fem_pmt_end; imod_fem_pmt++) {
      nword = 42;
      i = pcie_rec(hDevPMT,0,1,nword,iprint,py);     // init the receiver
      imod=imod_fem_pmt;                            // only read module next to the PMT xmit
      ichip=3;   // don't care about chip number for triggr module
      buf_send[0]=(imod<<11)+(ichip<<8)+mb_feb_pmt_rd_counters+(0x0<<16);  // read out status
      i=1;
      k=1;
    ...
    // PMT FEM NU BUFFER
    printf(" *****\033[95m Slow Ctrl Monitoring: \033[93mPMT-FEM NU/SN-Buffer Info \033[00m*****\n");
	...
	printf(" %-8d : %-8d :",
	       (((read_array[1] >> 16) & 0xffff)+ ((read_array[1] & 0xff)<< 16)),
	       ((read_array[1]>>8) &0xff));
      pmt_nublock[(int)((read_array[0]>>11) & 0x1f)] = ((read_array[1]>>8) &0xff);
      /*
	printf(" ***** Slow Ctrl Monitoring: PMT-FEM NU-Buffer Status Info *****\n");
	printf("    Neutrino buffer status word = %x, %x \n", read_array[0], read_array[1]);
	printf("    module addres %d, command %d \n",((read_array[0]>>11) & 0x1f) ,(read_array[0] & 0xff));
    ...
    i = pcie_rec(hDevPMT,0,2,nword,iprint,py);     // read out 2 32 bits words
    i = (read_array[0] >> 16);
    printf(" *****\033[95m Slow Ctrl Monitoring: \033[93mPMT-XMIT Status Info \033[00m*****\n");  
    ...
    */
    ...
    usleep(100);
    i = pcie_rec(hDevPMT,0,2,nword,iprint,py);     // read out 2 32 bits words
    printf(" ***** \033[95mSlow Ctrl Monitoring: \033[93mPMT-XMIT Counter Info \033[00m*****\n");
    //printf("    buffer status word = %x, %x, %x, %x, %x\n", read_array[0], read_array[1],
    //read_array[2], read_array[3], read_array[4]);
    //printf("    crate %d, header %x \n",(read_array[0] & 0x1f) ,((read_array[0] >> 8) & 0xff));
    printf("  frame count %d \n", read_array[1]);
    ...
    frame_ctr_pmt_xmit = read_array[1];
    event_ctr_pmt_xmit = read_array[2];
    read_ctr_pmt_xmit = read_array[3];
  }
  ...
  if(hDevPMT){
    ...
    imod=imod_trig;
    ichip=0;   // don't care about chip number for triggr module
    buf_send[0]=(imod<<11)+(ichip<<8)+mb_trig_rd_counter+(0x0<<16);  // read out status
    i=1;
    k=1;
    i = pcie_send(hDevPMT, i, k, px);
    usleep(10);
    ...
    printf("Div     : %d\n",((read_array[2]>>16) & 0x7));
    printf("Triggers: %d\n",read_array[3] & 0xffffff);
    printf("G1      : %d\n",read_array[4] & 0xffffff);
    printf("G2      : %d\n",read_array[5] & 0xffffff);
    printf("GFake   : %d\n",read_array[6] & 0xffffff);
    printf("Gate1   : %d\n",read_array[7] & 0xffffff);
    printf("Gate2   : %d\n",read_array[8] & 0xffffff);
    printf("GateFake: %d\n",read_array[9] & 0xffffff);
    printf("Trig1   : %d\n",read_array[10] & 0xffffff);
    printf("Calib   : %d\n",read_array[11] & 0xffffff);
    ...
    py = &read_array;
    i = pcie_rec(hDevPMT,0,2,nword,iprint,py);     // read out 2 32 bits words
    printf(" ***** \033[95mSlow Ctrl Monitoring: Trigger Status (ctr) Info \033[00m*****\n");
    //printf("    Header = %x\n", read_array[0]);
    printf(" Counter  : Frame    : 2 MHz    : 16 MHz\n");
    printf(" %-8d : %-8d : %-8d : %-8d\n",
	   (read_array[3] & 0xffffff),
	   (read_array[1] & 0xffffff),	
	   (read_array[2] & 0xfff),
    ...
    /*
      ...
      printf("  Trigger ctr    : %d\n", (read_array[3] & 0xffffff));
    */
    printf(" PMT Trigger 8-Bit Counters \n");
    for(i=4; i<12; i++)
      printf(" %-6d",read_array[i]);
    //printf("    PMT bit %d ctr  : %d\n", (i-4), read_array[i]);
    printf("\n\n");

    frame_ctr_trigger = (read_array[1] & 0xffffff);
    event_ctr_trigger = (read_array[3] & 0xffffff);
  }
  ...
  //
  static int   oframe,osample,odiv,cframe,csample,cdiv,ia,j;
...
#define  dma_abort 0x2
  ...
  //printf("\033[95miold=0x%x ... i=0x%x\033[00m ... start\n",iold,i);
  //
  //
  while ((iold - i) >= 16) {
    dwAddrSpace =cs_bar;
    ...
    //printf("\033[95miold=0x%x ... i=0x%x\033[00m ... loop start\n",iold,i);
    if((iold - i) >= 16) {
      if(ifr1 == 0) {
	gettimeofday(&start,NULL);
	...
	//printf("busy   %d \n", (read_array[0] & 0x1));
	new_trig_count++;
	printf("\033[1;33;40m[ NEW TRIGGER ]\033[00m\n");
	printf(" Trig: %-5d  Frame: %-5d  Sample: %-5d  Remine (16MHz): %-3d  Remine (64MHz): %-3d\n",
	       (read_array[1] >> 8),
	       ((read_array[1] & 0xff)<<16) + ((read_array[0] >>16)),
	...
	printf(" Bits: \033[1;35;40m");
	if((read_array[2]>>8) & 0x1) printf(" PC");
	if((read_array[2]>>9) & 0x1) printf(" EXT");
	if((read_array[2]>>12) & 0x1) printf(" Gate1");
	if((read_array[2]>>11) & 0x1) printf(" Gate2");
	if((read_array[2]>>10) & 0x1) printf(" Active");
	if((read_array[2]>>13) & 0x1) printf(" Veto");
	if((read_array[2]>>14) & 0x1) printf(" Calib");
	printf("\033[00m ... PMT data: 0x%x\n",(read_array[2] & 0xff));
      }
    }
    usleep(200);
    //printf("\033[95miold=0x%x ... i=0x%x\033[00m loop end\n",iold,i);
    if(iold == i) break;
  }
  return new_trig_count;
}
...
#define  mb_cntrl_set_trig1 0x4
  ...
  iused =0;
  nwrite_byte_s = dwDMABufSize_pmt;
  ...
  while (1) {
	...
	//        if(idebug ==1) printf(" use 4dw \n");
	  ...
	  if(write_point_s >= read_point_s) dis =  jbuf_ev_size_pmt - (write_point_s - read_point_s);
	  ...
	  if(write_point_s >= read_point_s_tmp) {
	    if((jbuf_ev_size_pmt - write_point_s) >= nwrite) {
		...
		buffer_ev_s[write_point_s+ik] = *buffp_rec32_s++;
	      ...
	      for (ik=0; ik< (jbuf_ev_size_pmt-write_point_s); ik++) {
		buffer_ev_s[write_point_s+ik] = *buffp_rec32_s++;
	      }
	      for (ik=0; ik< (nwrite-(jbuf_ev_size_pmt-write_point_s)); ik++) {
		buffer_ev_s[ik] = *buffp_rec32_s++;
	      }
	      write_point_s = write_point_s+ nwrite- jbuf_ev_size_pmt;
	    }
	  }
	  else {
	    for (ik=0; ik< nwrite; ik++) {
...
#define  dma_abort 0x2
  ...
  //
  //
  printf("PMT trigger thread started \n");
  my_data = (struct thread_data *) threadarg;
  taskid = my_data->thread_id;
  hDev1 = my_data->hDev;
  hDev = my_data->hDevc;
  ...
  iused =0;
  nwrite_byte_n = dwDMABufSize_pmt;
  ...
  while (1) {
	...
	u32Data = cs_init;
      ...
      /** start the receiver **/
      if(idebug == 1) printf(" PMT trigger -- DMA loop with DMA data length %d \n", nwrite_byte_n);
      dwAddrSpace = cs_bar;
      dwOffset = r_cs_reg;
    ...
    if(neu_buf_filled[iused] == 0) {
      ...
      if(idebug ==1) printf(" PMT trigger ---> DMA set up done, byte count = %d\n", nwrite_byte_n);
      //
      //
      //
      if(ith_fr == 1) {
	  ...
	  pthread_mutex_lock (&mutexlock);
	  if(write_point_n >= read_point_n) dis =  jbuf_ev_size_pmt - (write_point_n - read_point_n);
	  ...
	  if(write_point_n >= read_point_n_tmp) {
	    if((jbuf_ev_size_pmt - write_point_n) >= nwrite) {
	      for (ik=0; ik< nwrite; ik++) {
		buffer_ev_n[write_point_n+ik] = *buffp_rec32_n++;
	      ...
	      for (ik=0; ik< (jbuf_ev_size_pmt-write_point_n); ik++) {
		buffer_ev_n[write_point_n+ik] = *buffp_rec32_n++;
	      }
	      for (ik=0; ik< (nwrite-(jbuf_ev_size_pmt-write_point_n)); ik++) {
		buffer_ev_n[ik] = *buffp_rec32_n++;
	      }
	      write_point_n = write_point_n+ nwrite- jbuf_ev_size_pmt;
	    }
	  }
	  else {
	    for (ik=0; ik< nwrite; ik++) {
...
void *pt_trig_filewrite(void *nword_write)
{
  ...
  printf("PMT trigger file write thread started \n");
  while (1) {
    w_t1 = write_point_n;
    r_t1 = read_point_n;
    nwrite = dwDMABufSize_pmt/4;
    dis =w_t1 - r_t1;
    while (dis< nwrite) {
      w_t1 = write_point_n;
      r_t1 = read_point_n;
      dis =w_t1 - r_t1;
      if (dis < 0) dis = jbuf_ev_size_pmt + dis;
      usleep(300);
    }

    if((w_t1 > r_t1) | ((jbuf_ev_size_pmt -r_t1)>nwrite)) {
	...
	file_buf[is] = buffer_ev_n[is+r_t1];
      ...
      for (is=0; is<(jbuf_ev_size_pmt-r_t1); is++) {
	file_buf[is] = buffer_ev_n[is+r_t1];
      }
      index =0;
      for (is=(jbuf_ev_size_pmt-r_t1); is< (nwrite- (jbuf_ev_size_pmt-r_t1)); is++) {
    ...
    printf(" PMT Neutrino write point = %d, read point %d\n", write_point_n, read_point_n);
  }
}
...
#define  mb_cntrl_set_trig1 0x4
  ...
  iused =0;
  nwrite_byte_s = dwDMABufSize_tpc;
  nwrite = nwrite_byte_s/4;
  write_point_s_tpc = 0;
  read_point_s_tpc = 0;
  while (1) {
	...
	//        if(idebug ==1) printf(" use 4dw \n");
	  ...
	  //         printf("start array copy --- SN %d, %d\n",icopy,sn_buf_filled[icopy]);
	  pthread_mutex_lock (&mutexlock);
	  if(write_point_s_tpc >= read_point_s_tpc) dis =  jbuf_ev_size_tpc - (write_point_s_tpc - read_point_s_tpc);
	  ...
	  if(write_point_s_tpc >= read_point_s_tmp) {
	    if((jbuf_ev_size_tpc - write_point_s_tpc) >= nwrite) {
	      for (ik=0; ik< nwrite; ik++) {
		buffer_ev_s_tpc[write_point_s_tpc+ik] = *buffp_rec32_s++;
	      ...
	      for (ik=0; ik< (jbuf_ev_size_tpc-write_point_s_tpc); ik++) {
		buffer_ev_s_tpc[write_point_s_tpc+ik] = *buffp_rec32_s++;
	      }
	      for (ik=0; ik< (nwrite-(jbuf_ev_size_tpc-write_point_s_tpc)); ik++) {
		buffer_ev_s_tpc[ik] = *buffp_rec32_s++;
	      }
	      write_point_s_tpc = write_point_s_tpc+ nwrite- jbuf_ev_size_tpc;
	    }
	  }
	  else {
	    for (ik=0; ik< nwrite; ik++) {
...
#define  dma_abort 0x2
  ...
  iused =0;
  nwrite_byte_n = dwDMABufSize_tpc;
  nwrite = nwrite_byte_n/4;
  write_point_n_tpc = 0;
  read_point_n_tpc = 0;
  ...
  while (1) {
    for (is=1; is<3; is++) {
      ...
      dwAddrSpace = cs_bar;
      u32Data = cs_start+nwrite_byte_n;   /* 32 bits mode == 4 bytes per word *2 fibers **/
      if(idebug == 1) printf(" TPC trigger -- DMA loop with DMA data length %d \n", nwrite_byte_n);
      dwOffset = r_cs_reg;
      WDC_WriteAddr32(hDev1, dwAddrSpace, dwOffset, u32Data);
    ...
    if(neu_buf_filled[iused] == 0) {
      ...
      WDC_WriteAddr32(hDev1, dwAddrSpace, dwOffset, u32Data);
      if(idebug ==1) printf(" TPC trigger ---> DMA set up done, byte count = %d\n", nwrite_byte_n);
      //
      //
      //
      if(ith_fr == 1) {
	  ...
	  icopy = 0;
        ...
        if (neu_buf_filled[icopy] == 1) {
	  pthread_mutex_lock (&mutexlock);
	  if(write_point_n_tpc >= read_point_n_tpc) dis =  jbuf_ev_size_tpc - (write_point_n_tpc - read_point_n_tpc);
	  ...
	  if(write_point_n_tpc >= read_point_n_tmp) {
	    if((jbuf_ev_size_tpc - write_point_n_tpc) >= nwrite) {
	      for (ik=0; ik< nwrite; ik++) {
		buffer_ev_n_tpc[write_point_n_tpc+ik] = *buffp_rec32_n++;
	      ...
	      for (ik=0; ik< (jbuf_ev_size_tpc-write_point_n_tpc); ik++) {
		buffer_ev_n_tpc[write_point_n_tpc+ik] = *buffp_rec32_n++;
	      }
	      for (ik=0; ik< (nwrite-(jbuf_ev_size_tpc-write_point_n_tpc)); ik++) {
		buffer_ev_n_tpc[ik] = *buffp_rec32_n++;
	      }
	      write_point_n_tpc = write_point_n_tpc+ nwrite- jbuf_ev_size_tpc;
	    }
	  }
	  else {
	    for (ik=0; ik< nwrite; ik++) {
...
void *pt_trig_filewrite_tpc(void *nword_write)
{
  ...
  while (1) {
    ...
    nwrite = dwDMABufSize_tpc/4;
    dis =w_t1 - r_t1;
    while (dis< nwrite) {
      w_t1 = write_point_n_tpc;
      r_t1 = read_point_n_tpc;
      dis =w_t1 - r_t1;
      if (dis < 0) dis = jbuf_ev_size_tpc + dis;
      usleep(300);
    }

    if((w_t1 > r_t1) | ((jbuf_ev_size_tpc -r_t1)>nwrite)) {
      for (is=0; is<nwrite; is++) {
	file_buf[is] = buffer_ev_n_tpc[is+r_t1];
      ...
      for (is=0; is<(jbuf_ev_size_tpc-r_t1); is++) {
	file_buf[is] = buffer_ev_n_tpc[is+r_t1];
      }
      index =0;
      for (is=(jbuf_ev_size_tpc-r_t1); is< (nwrite- (jbuf_ev_size_tpc-r_t1)); is++) {
...
#define  dma_buffer_size        10000000
  ...
  */
  ...
  //
  //
  pmt_deadtime = 45; ///set deadtime long so it will not double fired on trailling edge
  pmt_mich_window = 2;
  //
  //
  threshold0 = 4095;   // 100 discriminator threshold 0
  threshold1 = 4095;   // 150 discriminator threshold 1
  cos_mult = 1;       // cosmic ray multiplicity threshold
  cos_thres = 10;    // cosmic ray sum ph threshold
  pmt_precount = 1; // set PMT precount
  pmt_words=0;      // number of word to read
  ...
  px = &buf_send;
  py = &read_array;
  printf("10 seconds to power-on\n");
  usleep(300000);  // wait for 200 ms
  imod=imod_fem;
  ichip =1;
  buf_send[0]=(imod<<11)+(ichip<<8)+0x1+(0x0<<16); //turn module 11 power on
  i=1;
  k=1;
  i = pcie_send(hDev, i, k, px);
  printf("20 seconds to power-off\n");
  //
  usleep(450000);  // wait for 200 ms

  imod=imod_fem;
  ichip =1;
  buf_send[0]=(imod<<11)+(ichip<<8)+0x0+(0x0<<16); //turn module 11 power off
  i=1;
  k=1;
  i = pcie_send(hDev, i, k, px);
  printf("20 seconds to power-on\n");
  //
  usleep(300000);  // wait for 200 ms
  ...
  i = pcie_send(hDev, i, k, px);
  //
  printf("20 seconds to load fpga\n");
  usleep(450000);  // wait for 200 ms
  //     printf(" enable number of loop\n");
  //     scanf("%d",&nloop);

  usleep(10000); // wait for 10ms
  //inpf = fopen("../../feb_pmt_fpga_old","r");
  inpf = fopen("../../feb_pmt_fpga_default","r");
  //inpf = fopen("../../feb_pmt_fpga_test","r");
  //if(imod_fem==4)
  //inpf = fopen("/datalocal/kterao/feb_pmt_fpga_00","r");
    //else
    //inpf = fopen("/datalocal/kterao/feb_pmt_fpga_01","r");
  imod=imod_fem;
  ichip=mb_feb_conf_add;
  buf_send[0]=(imod<<11)+(ichip<<8)+0x0+(0x0<<16);  // turn conf to be on
  i=1;
  ...
  printf("Read & send time: %d ... (%d send)",clock()-start_time,kazu_ctr);
  usleep(2000);    // wait for 2ms to cover the packet time plus fpga init time
  fclose(inpf);
  //
  //
  ...
  imod=imod_fem;
  ichip=3;
  //buf_send[0]=(imod<<11)+(ichip<<8)+mb_feb_pmt_gate_size+(0x16<<16);  // set gate size (0x64 = 100 samples
  //buf_send[0]=(imod<<11)+(ichip<<8)+mb_feb_pmt_gate_size+(0x64<<16);  // set gate size (0x64 = 100 samples
  //buf_send[0]=(imod<<11)+(ichip<<8)+mb_feb_pmt_gate_size+(0xfa<<16);  // set gate size (0x64 = 100 samples
  //buf_send[0]=(imod<<11)+(ichip<<8)+mb_feb_pmt_gate_size+(0x5dc<<16);  // set gate size (0x5dc = 1500 samples)
  buf_send[0]=(imod<<11)+(ichip<<8)+mb_feb_pmt_gate_size+(0x7d0<<16);  // set gate size (0x5dc = 1500 samples)
  //buf_send[0]=(imod<<11)+(ichip<<8)+mb_feb_pmt_gate_size+(0xbb8<<16);  // set gate size (0xbb8 = 3000)
  i=1;
  k=1;
  i = pcie_send(hDev, i, k, px);
  //
  ...
  imod=imod_fem;
  ichip=3;
  buf_send[0]=(imod<<11)+(ichip<<8)+mb_feb_pmt_beam_delay+(0x10<<16);  
  //buf_send[0]=(imod<<11)+(ichip<<8)+mb_feb_pmt_beam_delay+(0x1<<16);  
  i=1;
  k=1;
  i = pcie_send(hDev, i, k, px);
  //
  ...
  imod=imod_fem;
  ichip=3;
  buf_send[0]=(imod<<11)+(ichip<<8)+mb_feb_pmt_beam_size+(0x40<<16);  
  i=1;
  k=1;
  i = pcie_send(hDev, i, k, px);
  //
  ...
  imod=imod_fem;
  ichip=3;
  //buf_send[0]=(imod<<11)+(ichip<<8)+mb_feb_pmt_trig_delay+(0x1<<16);  
  buf_send[0]=(imod<<11)+(ichip<<8)+mb_feb_pmt_trig_delay+(0xf<<16);  
  i=1;
  k=1;
  i = pcie_send(hDev, i, k, px);
  ...
  imod=imod_fem;
  ichip=3;
  //buf_send[0]=(imod<<11)+(ichip<<8)+mb_feb_pmt_gate1_size+(0x16<<16);  // set gate size (0x64 = 100)
  //buf_send[0]=(imod<<11)+(ichip<<8)+mb_feb_pmt_gate1_size+(0x64<<16);  // set gate size (0x64 = 100)
  //buf_send[0]=(imod<<11)+(ichip<<8)+mb_feb_pmt_gate1_size+(0xfa<<16);  // set gate size (0xfa = 250)
  //buf_send[0]=(imod<<11)+(ichip<<8)+mb_feb_pmt_gate1_size+(0x5dc<<16);  // set gate size (0x5dc = 1500)
  buf_send[0]=(imod<<11)+(ichip<<8)+mb_feb_pmt_gate1_size+(0x7d0<<16);  // set gate size (0x5dc = 1500)
  //buf_send[0]=(imod<<11)+(ichip<<8)+mb_feb_pmt_gate1_size+(0xbb8<<16);  // set gate size (0xbb8 = 3000)
  i=1;
  k=1;
  i = pcie_send(hDev, i, k, px);
  //
  ...
  imod=imod_fem;
  ichip=3;
  buf_send[0]=(imod<<11)+(ichip<<8)+mb_feb_pmt_beam1_delay+(0x10<<16);  
  buf_send[0]=(imod<<11)+(ichip<<8)+mb_feb_pmt_beam1_delay+(0x1<<16);  
  i=1;
  k=1;
  i = pcie_send(hDev, i, k, px);
  //
  ...
  imod=imod_fem;
  ichip=3;
  buf_send[0]=(imod<<11)+(ichip<<8)+mb_feb_pmt_beam1_size+(0x40<<16);  
  i=1;
  k=1;
  i = pcie_send(hDev, i, k, px);
  //
  ...
  imod=imod_fem;
  ichip=3;
  //buf_send[0]=(imod<<11)+(ichip<<8)+mb_feb_pmt_trig1_delay+(0x1<<16);  
  buf_send[0]=(imod<<11)+(ichip<<8)+mb_feb_pmt_trig1_delay+(0xf<<16);  
  i=1;
  k=1;
  i = pcie_send(hDev, i, k, px);
  ...
  //printf(" enter 1 to continue FPGA ADC receiver reset\n");
  //    scanf("%d",&ik);
  printf("Continue to FPGA ADC receiver reset\n");
  ...
  //printf(" enter 1 to continue FPGA ADC receiver align\n");
  //    scanf("%d",&ik);
  printf("Continue to FPGA ADC receiver align\n");
  usleep(1000);
  ...
  //    scanf("%d",&ik);
  printf("Continue to ADC alignment \n");  
...
#define  dma_buffer_size        10000000
  ...
  */
  ...
  for (imod_fem = imod_xmit+1; imod_fem <= imod_st; ++imod_fem) {
    imod=imod_fem;
    printf(" reset the link PLL for module %x \n", imod);
    ichip=4;
    buf_send[0]=(imod<<11)+(ichip<<8)+mb_feb_pll_reset+(0x0<<16);    // reset LINKIN PLL
