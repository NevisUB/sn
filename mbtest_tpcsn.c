...
/* -----------------------------------------------
   ...
   ----------------------------------------------- */
static void MenuMain(WDC_DEVICE_HANDLE *phDev, WDC_DEVICE_HANDLE *phDev1, 
		     WDC_DEVICE_HANDLE *phDev2, WDC_DEVICE_HANDLE *phDev3);
...
static void MenuMBtest(WDC_DEVICE_HANDLE hDev,WDC_DEVICE_HANDLE hDev1 ,
		       WDC_DEVICE_HANDLE hDev2, WDC_DEVICE_HANDLE hDev3,
		       WDC_DEVICE_HANDLE hDev4, WDC_DEVICE_HANDLE hDev5);
...
int main(void)
{
  ...
  printf("\nStart time of program: %ld sec %ld usec\n",seconds,useconds);
    ...
    //    if (PCIE_DEFAULT_VENDOR_ID)
    //        hDev1 = DeviceFindAndOpen(PCIE_DEFAULT_VENDOR_ID, PCIE_DEFAULT_DEVICE_ID); // Jose: commented
    if (PCIE_DEFAULT_VENDOR_ID)
      hDev2 = DeviceFindAndOpen(PCIE_DEFAULT_VENDOR_ID, PCIE_DEFAULT_DEVICE_ID+1); //Jose: +1 added
    //if (PCIE_DEFAULT_VENDOR_ID)
...
*/
    ...
    static UINT32 send_array[40000],read_array[dma_buffer_size],read_array_w[dma_buffer_size],read_array1[40000];
...
//
    ...
    case 1:
     ...
     //     scanf("%d",&nloop);//this is for xmit config...
     nloop=1;
     printf(" enter number of DMA loops to do\n");
     scanf("%d",&ndma_loop);
     ...
     if (dwDMABufSize>dma_buffer_size){ printf("too large buffer size! will have issues with array size\n"); break; }

     fd = creat("test123_sn.dat",0755);
...
//     scanf("%d",&nloop);
     for (j=0; j<nloop; j++) {
       ...
       //inpf = fopen("/home/kterao/local152_code/kazu_test1_012014/../xmit_fpga_link_header","r");
       imod=imod_xmit;
     ...
     printf(" xmit done, enter 1 to boot FEM \n");
     scanf("%d",&ik);
     ...
     // inpf = fopen("/home/kterao/local152_code/kazu_test1_012014/../feb_tpc_fpga_sn_zero_test","r");
     ...
     fclose(inpf);
     //
     // Jose: try 3 times to get FEM status since first could be wrong
     int tryRead;
     for(tryRead=1; tryRead<=3; tryRead++){
       ...
       if(iprint == 1) printf("Reading FEM module %d status trial %d \n", imod, tryRead);
       if(iprint == 1) printf("FEM module %d status word after boot = %x, %x \n", imod, read_array[0], read_array[1]);

       int emptySlot = 13; // Jose: must be an empty slot
       // Jose: pass NULL as hDevPMT
       check_status(NULL,hDev,emptySlot,emptySlot,emptySlot,imod_fem,imod_fem,emptySlot,imod_xmit);
       if(iprint == 1) printf("Finished check_status \n");
       usleep(3000);    // wait for 3ms to cover the packet time plus fpga init time
     }
     ...
     ichip=1;
     iframe= 8191;    //1023
     // iframe=25599; // Jose: originally was this
     // iframe=2047; 
	 ...
	 //         else ijk=ibase;
         ...
         iround = iround + idir;
         ijk= ibase + iround;
	 //         if( (ik > 240) & (ik <245)) ijk=300+ibase;
         // if ((ik>247)) ijk=300+ibase;
	 //         if ( ( ik > 100 && ik < 105) ) ijk=200+ibase;
         //vic
	 if ( ( ik > 100 && ik < 105) ) ijk=200+ibase;
         if ( ( ik > 90  && ik < 95) ) ijk=200+ibase;
         if ( ( ik > 110 && ik < 115) ) ijk=200+ibase;
         if ( ( ik > 125 && ik < 130) ) ijk=200+ibase;
         // if ( ( ik < 3 ) || ( ik > 254 ) ) ijk=300+ibase;
         if(is == 0) {
     ...
     //
     ijk=10;
     buf_send[0]=(imod<<11)+(ichip<<8)+(mb_feb_tpc_load_thr_mean)+((ijk & 0xffff)<< 16); // load mean
     i = pcie_send(hDev, 1, 1, px);
     usleep(10);
     
     ijk=100;
     buf_send[0]=(imod<<11)+(ichip<<8)+(mb_feb_tpc_load_thr_vari)+((ijk & 0xffff)<< 16); // load variance
     i = pcie_send(hDev, 1, 1, px);
     usleep(10);
     ...
     printf(" finish XMIT alignment, enter 1 to continue \n");
     scanf("%d",&ik);
     //
     //
     //
	...
	//     copy of the data array, one for comparison, the other to write to file
	//
	        for (is=0; is<nwrite*2; is++) {
		  read_array[is+(iv*(nwrite*2))]= *buffp_rec32++;
		  read_array_w[is] = read_array[is+(iv*(nwrite*2))];
	        }
		n_write = write(fd,read_array_w,nwrite*2*4);
     ...
     if(irawprint ==1) {
       for (is=0; is< ntot_rec; is++) {
         ...
         if(((is%250000) ==0) && (is !=0)) {
	   printf(" type 1 to continue with printing\n");
	   scanf("%d", &dummy1);
         }
     ...
     if(comp_s == 1) {
	   ...
	   if(read_comp[ik] != read_array[k]) {
	     ...
	     printf("enter 1 to acknowledge data error and continue\n");
	     scanf("%d",&k);
	 ...
	 is = is+2;
	 printf("done comparing, enter 1 to continue\n");
	 scanf("%d", &ik);
       }
     }
    ...
    case 2:
     ...
     scanf("%d",&ineu);
     printf(" enter 1 to to disbale module 0 channel 0 \n");
     scanf("%d",&idis_c);
     printf(" enter 1 to to disbale module 1 channel 0 \n");
     scanf("%d",&idis_c1);
     printf(" enter buffer size in bytes \n");
     scanf("%d",&dwDMABufSize);
...
//
           if(ihuff == 1) {
            if((idis_c != 1) | (imod != (imod_xmit+1)) | (is !=0)){
             if((idis_c1 != 1) | (imod != (imod_xmit+2)) | (is !=0)){
...
//         u32Data = cs_start+(nwrite*2)*4;   /* 32 bits mode == 4 bytes per word *2 fibers **/
          ...
          if((iwrite != 1) & (idebug ==1)) printf(" DMA loop %d with DMA data length %d \n", iv, nwrite_byte);
          dwOffset = r_cs_reg;
          WDC_WriteAddr32(hDev2, dwAddrSpace, dwOffset, u32Data);
         }
//         ifr=1;
...
/* write this will start DMA */
         ...
         if(is == 0) {
          if((iwrite != 1) & (idebug ==1)) printf(" use 3dw \n");
          u32Data = dma_tr12+dma_3dw_rec;
         }
         else {
          u32Data = dma_tr12+dma_4dw_rec;
          if((iwrite != 1) & (idebug ==1)) printf(" use 4dw \n");
         }
         WDC_WriteAddr32(hDev2, dwAddrSpace, dwOffset, u32Data);
...
/***    check to see if DMA is done or not **/
         ...
         for (is=0; is<60000; is++) {;
          ...
          if((u32Data & dma_in_progress) == 0) {
           idone =1;
//           if(idebug ==1) printf(" receive DMA complete %d \n", i);
           if((iwrite != 1) & (idebug ==1)) printf(" receive DMA complete %d \n", is);
          }
          if((u32Data & dma_in_progress) == 0) break;
         }
         if(idone == 0) {
...
//
         ...
         for (is=0; is<nwrite; is++) {
          read_array[is+ntot_rec]= *buffp_rec32++;
         }
         ntot_rec = ntot_rec+nwrite;
        }
        if((ijk%10000) ==0) printf(" event %d totoal data receive %d ,current event checked %d , event pointer %d \n", ijk, ntot_rec, event_ev, (event_save+jevent-1));
        if(ineu == 1) {
         if(((ijk+1)%2) == 0)  {
          imod=0;
          ichip=1;
...
//
        if(comp_s == 1) {
         if (ifr_c2 == 0) {
          ifr_c2 = 1;
          nword_comp =1;
          read_comp[0] = 0xffffffff;
          for (is =1; is< ntot_rec; is++) {
           nword_comp = nword_comp+1;
           read_comp[is] = read_array[is];
           if(read_array[is] == 0xffffffff) {
            nword_comp = nword_comp-1;
            ic = nword_comp;
            icomp_index = 0;
            event_save = ((read_array[3]>>16) & 0xfff)+((read_array[3] &0xfff) <<12);
            frame_save = ((read_array[4]>>16) & 0xfff)+((read_array[4] &0xfff) <<12);
            printf(" event size = %d, event = %d, frame = %d\n", nword_comp, event_save, frame_save);
//            scanf("%d",&ik);
            break;
           }
          }
         ...
         if((iwrite != 1) & (idebug ==1)) printf("ic = %d, %d\n", ic, icomp_index);
         while (ic < ntot_rec) {
//          printf(" %d, %d, %x, %x \n",ic,icomp_index, read_array[ic], read_comp[icomp_index]);
//          scanf("%d",&ik);
//
//        only checking the 1st module eveny number, frame number
//
          if(read_array[ic] == 0xffffffff) {
            ic_ev = ic+3;
            if(ic_ev >= ntot_rec) ic_ev = ic_ev-ntot_rec;
            ic_fr = ic+4;
            if(ic_fr >= ntot_rec) ic_fr = ic_fr-ntot_rec;
//            printf(" event = %d, ic_ev = %d, \n", ijk,ic_ev);
          }
          if(ic == ic_ev) {
            event_ev = ((read_array[ic]>>16) & 0xfff)+((read_array[ic] &0xfff) <<12);
            if(event_ev != ((event_save+jevent) & 0xffffff)) {
             printf(" event = %d, should be %d\n", event_ev, (event_save+jevent));
            }
          }
          if(ic == ic_fr) {
            frame_ev = ((read_array[ic]>>16) & 0xfff)+((read_array[ic] &0xfff) <<12);
            if(frame_ev != ((frame_save+jevent) & 0xffffff)) {
             printf(" frame = %d, should be %d\n", frame_ev, (frame_save+jevent));
            }
          }

          if((read_array[ic] != read_comp[icomp_index]) & ((read_array[ic] & 0xf000f000) != 0xf000f000)) {
            if(read_array[ic] == read_comp[icomp_index+1]) {
              if(icomp_index != 5) printf("** %d\n", icomp_index);
              else printf("*");
              if(icomp_index < (nword_comp-1)) icomp_index = icomp_index+1;
              else {
                jevent = jevent+1;
                icomp_index =0;
              }
             ...
             printf(" event = %d, index = %d, icomp_index = %d, data = %x, 1st event = %8x \n", ijk, ic,icomp_index, read_array[ic], read_comp[icomp_index]);
             for (ib=0; ib<200; ib++) {
              printf("%8x ", read_array[ic+ib-50]);
              if((ib+1)%8 == 0) printf("\n");
             }
             printf("\n");
             for (ib=0; ib<200; ib++) {
              nk = icomp_index+ib-50;
              if(nk >= 0)  printf("%8x ", read_comp[nk]);
...
//            printf(" %x, %x, %x, %x %x\n", read_array[ic-2],read_array[ic-1],read_array[ic],read_array[ic+1],read_array[ic+2]);
//            printf(" %x, %x, %x, %x %x\n", read_comp[icomp_index-2], read_comp[icomp_index-1],read_comp[icomp_index],read_comp[icomp_index+1],read_comp[icomp_index+2]);
          }
          ic = ic+1;
          if(icomp_index < (nword_comp-1)) icomp_index = icomp_index+1;
          else {
...
//          scanf("%d",&ik);
        ...
        if(irawprint ==2) {
         ...
         while (is < (ntot_rec-1) ) {
          ...
          k= read_array[is] & 0xffff;
          if(k == 0xffff) {
           printf("receive data word = %x, %x, %x, %x, %x\n", read_array[is], read_array[is+1], read_array[is+2], read_array[is+3], read_array[is+4]);
           printf(" module header word %x \n",(read_array[is] & 0xffff));
           k=(read_array[is]>>16) & 0xfff;
           printf(" module adress %d, id number %d\n", (k & 0x1f), ((k>>5) & 0x7f));
           printf(" number of data word to read %d\n", (((read_array[is+1]>>16) & 0xfff)+((read_array[is+1] &0xfff) <<12)));
           printf(" event number %d\n", (((read_array[is+2]>>16) & 0xfff)+((read_array[is+2] &0xfff) <<12)));
           printf(" frame number %d\n", (((read_array[is+3]>>16) & 0xfff)+((read_array[is+3] &0xfff) <<12)));
           printf(" checksum %x\n", (((read_array[is+4]>>16) & 0xfff)+((read_array[is+4] &0xfff) <<12)));
           is = is+5;
          }
          if(read_array[is] == 0xe0000000) {
            printf(" end of event %x \n", read_array[is]);
            is =is+1;
          }
          read_array_s[0] = (read_array[is] & 0xffff);
          read_array_s[1] = ((read_array[is]>>16) & 0xffff);
          for (i=0; i<2; i++) {
           ...
           else if ((read_array_s[i] & 0xf000) == 0x5000) {
            iset=0;
            printf(" channel end %d\n",(read_array_s[i] &0xfff));
           }
           else if (iset ==1) {
            if((read_array_s[i] & 0x8000) == 0x8000) {   // compressed event
//             printf(" enter decode \n");
             ib=0;
             while (ib< 15) {
//              printf("ib = %d, read_array_s[i] = %x,  (read_array_s[i]>> ib) = %x \n", ib, read_array_s[i],  (read_array_s[i]>> ib));
              if(((read_array_s[i]>> ib) & 0x1) == 1) {    // looking for the next 1
               if (ib == 14) {
                 dis =0;
                 ib = ib+1;
               }
                ...
                dis =0;
                for (ic=ib+1; ic< 15; ic++) {
                 if(((read_array_s[i]>> ib) & 0x1) == 0) dis =dis+1;
                 else {
                  ib =ic;
                  break;
                 }
...
//              printf("11 ib = %d, ic = %d, dis =%d, sample_b = %d \n",ib,ic,dis, sample_b);
//              scanf("%d",&ik);
              if(dis == 0) sample_b = sample_b;
              else if(dis == 1) {
                sample_b = sample_b-1;
              }
              else if(dis == 2) {
              ...
              ncount = ncount+1;
              printf(" %4x",sample_b);
              if((ncount%16) == 0) printf("\n");
              }
              else ib=ib+1;

             }
//             printf("ib = %d, ic_s = %d, dis =%d \n",ib,ic_s,dis);
            }
            else {
             printf(" %4x",read_array_s[i]);
             ncount = ncount+1;
             sample_b = read_array_s[i];       // keep the sample before
             if((ncount%16) == 0) printf("\n");
            }
         ...
         printf(" trailer word = %x \n", read_array[ntot_rec-1]);
        }
        if(irawprint ==1) {
         for (is=0; is< ntot_rec; is++) {
          if((is%8) ==0) printf("%4d",is);
    ...
    case 3:
     printf(" MicroBoon neutrino events \n");
...
//     timesize =2000;
     timesize =20;
     dwDMABufSize = 800000;
     printf(" frame size = %d, drift time %d \n", iframe_length, timesize);
//     timesize = 3199;
...
//       scanf("%d",&ik);
 //      ik =1;
     for (imod_fem = (imod_xmit+1); imod_fem< (imod_st+1); imod_fem++) {
       imod=imod_fem;
       printf(" load parameter for module %d \n", imod);
       ichip=3;
       buf_send[0]=(imod<<11)+(ichip<<8)+mb_feb_dram_reset+(0x1<<16);  // turm the DRAM reset on
       i=1;
       k=1;
...
//
//    unlike the supernova we start the run 1ss
//
...
//
      ...
      printf(" aaaa\n");
      ndma_loop = (nword*4)/dwDMABufSize;
      if(ifr ==0) printf(" DMA will run %d loop\n per events", (ndma_loop+1));
...
//
        ...
        if(iv == 0) {
         ...
         k=1;
         i = pcie_send(hDev, i, k, px);
         if(iwrite !=1 ) printf(" trigger send  \n");
        }
        if(irawprint==1) scanf("%d", &i);
...
/***    check to see if DMA is done or not **/
        ...
        for (is=0; is<600000; is++) {;
         ...
         if((u32Data & dma_in_progress) == 0) {
           idone =1;
//           if(idebug ==1) printf(" receive DMA complete %d \n", i);
           if(iwrite !=1 ) printf(" receive DMA complete %d \n", is);
         }
         if((u32Data & dma_in_progress) == 0) break;
        }
        if(idone == 0) {
...
//
       ...
       if((ijk%40) ==0)printf(" event %d totoal data receive %d \n", ijk, ntot_rec);
//
//
//
//       printf(" buffer 1 data print \n");
//       buffp_rec32 = pbuf_rec1;
//       for (is=0; is< (dwDMABufSize/4); is++) {
//        if((is%8) ==0) printf("%4d",is);
//        if(((is%25000) ==0) && (is !=0)) {
//         printf(" type 1 to continue\n");
//         scanf("%d", &dummy1);
//        }
//        printf(" %8x",*buffp_rec32++);
//        if(((is+1)%8) ==0 ) printf("\n");
//       }
//       if(((is+1)*8) != 0)printf("\n");
//
//
//
//       printf(" buffer 2 data print \n");
//       buffp_rec32 = pbuf_rec2;
...
//
       ...
       if(iwrite ==1 ) {
        send_array[0] = ijk;
        send_array[1] = ntot_rec;
        n_write = write(fd,send_array,8);
//        printf("n_write = %d \n", n_write);
        n_write = write(fd,read_array,(ntot_rec*4));
//        printf("n_write = %d \n", n_write);
       }
...
//         ifr=1;
         printf(" buffer allocation 1\n");
         dwStatus = WDC_DMAContigBufLock(hDev1, &pbuf_rec1, dwOptions_rec, dwDMABufSize, &pDma_rec1);
         if (WD_STATUS_SUCCESS != dwStatus) {
          printf("Failed locking a rec 1 Contiguous DMA buffer. Error 0x%lx - %s\n", dwStatus, Stat2Str(dwStatus));
          printf("enter 1 to continue \n");
          scanf("%d",&is);
...
//         ifr=1;
         printf(" buffer allocation 2\n");
         dwStatus = WDC_DMAContigBufLock(hDev1, &pbuf_rec2, dwOptions_rec, dwDMABufSize, &pDma_rec2);
         if (WD_STATUS_SUCCESS != dwStatus) {
          printf("Failed locking a rec 2 Contiguous DMA buffer. Error 0x%lx - %s\n", dwStatus, Stat2Str(dwStatus));
          printf("enter 1 to continue \n");
          scanf("%d",&is);
...
/***    check to see if DMA is done or not **/
        ...
        for (is=0; is<6000000; is++) {;
         ...
         if((u32Data & dma_in_progress) == 0) {
           idone =1;
           if (is > wait_max) wait_max =is;
           if (is < wait_min) wait_min =is;
//           if(idebug ==1) printf(" receive DMA complete %d \n", i);
//**           if(iwrite !=1 ) printf(" receive DMA complete %d \n", is);
         }
         if((u32Data & dma_in_progress) == 0) break;
        ...
        if(idone == 0) {
         printf(" loop %d out of %d, Event %d DMA is not finished \n", iv,ndma_loop,ijk);
         ...
         for (ig=1; ig<3; ig++) {
          ...
          u32Data=u64Data;
          printf (" transmitter %d status = %8X \n",ig,u32Data);
          u32Data=u64Data>>32;
          printf (" receiver %d status = %8X \n",ig,u32Data);
          if(ig == 1) nremain_tran1 = u32Data;
          else nremain_tran2 = u32Data;
         }
         ...
         dwOffset = cs_dma_cntrl;
         WDC_ReadAddr32(hDev1, dwAddrSpace, dwOffset, &u32Data);
         printf(" receive DMA status word before abort %X \n", u32Data);

         dwAddrSpace =cs_bar;
         dwOffset = cs_dma_by_cnt;
         WDC_ReadAddr32(hDev1, dwAddrSpace, dwOffset, &u32Data);
         printf (" DMA byte count before abort signal = %8X \n",u32Data);

         nremain_dma = u32Data;
         nremain = (u32Data - nremain_tran1)/4;
         nread_dma = (nwrite_byte/4) - nremain - (nremain_tran1/4);
         printf(" nremain = %d nread_dma = %d, word count = %d %d\n", nremain, nread_dma, (nwrite_byte)/4, (u32Data/4));

         dwAddrSpace =cs_bar;
         dwOffset = cs_dma_by_cnt;
         WDC_ReadAddr64(hDev1, dwAddrSpace, dwOffset, &u64Data);
         printf (" DMA word count = %x, %x \n",(u64Data>>32), (u64Data &0xffffffff));
//         nremain_dma = u64Data & 0xffffffff;
//
         dwAddrSpace =cs_bar;
         u64Data =0;
         dwOffset = t1_cs_reg;
...
//
       ...
       if((ijk%200) ==0) {
         printf(" event %d totoal data receive %d wait_max %d wait_min %d\n", ijk, ntot_rec, wait_max, wait_min);
       }  
//
//
//
...
//        printf("n_write = %d \n", n_write);
        n_write = write(fd,read_array,(ntot_rec*4));
//        printf("n_write = %d \n", n_write);
//         if(read_array[ntot_rec-1] != 0xe0000000) {
//          printf("event %d, last word %8x \n", ijk,read_array[ntot_rec-1]);
//         }
       }
       if(ilast_check == 1) {
        if(read_array[ntot_rec-1] != 0xe0000000) {
...
//&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&
    ...
    case 6:
     ...
     printf(" type 1 for the new header \n");
     scanf("%d", &inew);
//     printf(" enter number of words per packet \n");
//     scanf("%d",&nsend);
//     printf(" enable number of loop\n");
//     scanf("%d",&nloop);
     ...
     printf(" number event \n");
     scanf("%d",&nevent);
     printf(" type 1 for using ADC \n");
     scanf("%d", &itpc_adc);
     printf(" type 1 to test zero suppression code \n");
     scanf("%d", &izero);
     if(izero == 1) {
       ibase =100;
       islope = 20;
       ...
       ibeg = 20;
       if(izero == 1) {
        printf(" enter 1 to run the zero suppression \n");
        scanf("%d", &izero_sup);
       }
     }
...
//     scanf("%d",&nloop);
     for ( j=0; j<nloop; j++) {
      usleep(10000); // wait for 10ms
      if(inew == 1) {
        if(izero == 0) inpf = fopen("/home/ub/feb_fpga_test_new_head","r");
        else inpf = fopen("/home/ub/feb_fpga_test_new_head_zero","r");
      }
      else inpf = fopen("/home/ub/feb_fpga_test","r");
      imod=imod_fem;
      ichip=mb_feb_conf_add;
      buf_send[0]=(imod<<11)+(ichip<<8)+0x0+(0x0<<16);  // turn conf to be on
      i=1;
...
//         else ijk=ibase;
         ijk= ibase;
         if( (ik > 230) & (ik <235)) ijk=300+ibase;
         k = 0x8000+ ijk;        // make sure bit 15-12 is clear for the data
 //        printf("k = %x\n", k);
 //        scanf("%d", &ijk);
         buf_send[0]=(imod<<11)+(ichip<<8)+(mb_feb_test_ram_data)+((k & 0xffff)<<16); // load test data
...
//
       ...
       i=1;
       k=1;
//       if(izero == 0) i = pcie_send(hDev3, i, k, px);
...
//
      ...
      imod=imod_fem;
      ichip=3;
      buf_send[0]=(imod<<11)+(ichip<<8)+mb_feb_a_nocomp+(0x0<<16);  // set a channel no compression
      i=1;
      k=1;
      i = pcie_send(hDev3, i, k, px);
...
//      il == word count
//      ia == current ADC data word
//       k == bit sequence index
//      ic == bit sequence index after finding no-zero bit
//     ijk == how many zero in the huffman coded data
//
       if(izero == 1) {
        for(i=0; i< 2*nword; i++) {
         ik= (read_array_s[i] & 0xf000) >> 12;
         j= read_array_s[i];
//         printf("\n j= %x, ik= %x, ia = %x \n",j,ik,ia);
//         printf(" enter 1 to continue \n");
//         scanf("%d",&is1);
       ...
       if((iprint ==1) & (izero != 1)) {
        ...
        for(i=0; i< 2*nword; i++) {
         if((read_array_s[i] & 0xf000) == 0x4000) {
           ...
           if((ncount%8) == 0) printf("%6d", ncount);
           printf(" %4x",read_array_s[i]);
           ncount = ncount+1;
...
/* Initialize and set thread detached attribute */
//
     pthread_attr_init(&attr_pt_fake);
     pthread_attr_setdetachstate(&attr_pt_fake, PTHREAD_CREATE_JOINABLE);
     pthread_attr_getstacksize (&attr_pt_fake, &stacksize);
     printf("Send Fake defualt stack size = %li\n", stacksize);
     stacksize = 4*stacksize;
//      printf("Amount of stack needed per thread = %li\n",stacksize);
     pthread_attr_setstacksize (&attr_pt_fake, stacksize);
//      printf("Creating threads with stack size = %li bytes\n",stacksize);
//
//
//       allocate Neutrino Buffer 1
//
     printf(" buffer allocation --- Nfake data send\n");
     dwStatus = WDC_DMAContigBufLock(hDev3, &pbuf_send_fake, dwOptions, dwDMABufSize, &pDma_send_fake);
     if (WD_STATUS_SUCCESS != dwStatus) {
      printf("Failed locking a Neutrino Rec Contiguous DMA buffer 1. Error 0x%lx - %s\n", dwStatus, Stat2Str(dwStatus));
      printf("enter 1 to continue \n");
      scanf("%d",&is);
...
/* write this will abort previous DMA */
     ...
     dwOffset = 0x10;
     u32Data = 0x2;
     WDC_WriteAddr32(hDev3, dwAddrSpace, dwOffset, u32Data);
/* clear DMA register after the abort */
     dwAddrSpace =2;
     dwOffset = 0x10;
     u32Data = 0;
...
//     istart = 0x555;
     iloop =0;
     buffp_send = pbuf_send_fake;
     for (i=0; i< 512000; i++) {
//      *buffp_send++ = ((istart) + ((istart+1)<<16)) & 0xfff0fff;
      if((i%32) ==0) {
        istart_old = istart_old+1;
        istart = istart_old;
      }
      *buffp_send++ = ((istart) + ((istart+1)<<16)) & 0xfff0fff;

//      if(i%64 == 0) {
//       if(istart == 0x555) istart = 0xaaa;
//       else istart = 0x555;
//      }
      istart= istart+2;
//      if(istart ==64) {
...
//      }
     ...
     printf(" enter 1 write data to the file\n");
     scanf("%d", &iwrite);
     if(iwrite ==1) outf = fopen("/home/ub/test.dat","w");
     px = &buf_send;
     py = &read_array;
...
//
     imod=imod_fem;
     ichip=5;
     buf_send[0]=(imod<<11)+(ichip<<8)+mb_pmt_adc_reset+(0x1<<16);  // FPGA ADC receiver reset on
     i=1;
     k=1;
     i = pcie_send(hDev, i, k, px);
...
//
     imod=imod_fem;
     ichip=5;
     buf_send[0]=(imod<<11)+(ichip<<8)+mb_pmt_adc_reset+(0x0<<16);  // FPGA ADC receiver reset off
     i=1;
     k=1;
     i = pcie_send(hDev, i, k, px);
     usleep(1000);
...
//     for ( j=0; j<nloop; j++) {
     usleep(10000); // wait for 10ms
     inpf = fopen("/home/ub/feb_fpga_testmodule","r");
     imod=imod_fem;
     ichip=mb_feb_conf_add;
     buf_send[0]=(imod<<11)+(ichip<<8)+0x0+(0x0<<16);  // turn conf to be on
     i=1;
...
//
     printf(" ready to set no compressin \n");
     scanf("%d",&ik); 
     imod=imod_fem;
     ichip =3;
...
//
     ...
     ichip=3;
//                  -- number of FEM module -1, counting start at 0
     buf_send[0]=(imod<<11)+(ichip<<8)+mb_xmit_modcount+(0<<16);
     i=1;
     k=1;
     i = pcie_send(hDev, i, k, px);
//
...
//
     imod=imod_xmit;
     ichip=3;
     buf_send[0]=(imod<<11)+(ichip<<8)+mb_xmit_enable_1+(0x0<<16);  // disable token 1 pass
...
/* write this will start DMA */
     ...
     if(ifr ==0) {
      ifr=1;
      thread_data_n.id = 0;
      thread_data_n.hdev = hDev3;
      thread_data_n.hdevc = hDev3;
      rc_pt = pthread_create(&threads[0], &attr_pt_fake, pcie_send_fake, (void *)&thread_data_n);
      printf("  run fake data thread \n");
...
//
     ...
     while (is < 200000) {
//     printf(" is=%d, read_array = %8x \n", is, read_array[is]);
//     scanf("%d",&ik);
      if(read_array[is] == 0xffffffff) {
        if(iwrite !=1 ) printf(" event header ");
        else fprintf(outf," event header ");
        for (i=0; i<7; i++) {
         if(iwrite !=1 ) printf(" %8x", read_array[is+i]);
         else fprintf(outf," %8x", read_array[is+i]);
        }
        if(iwrite !=1 ) printf("\n");
        else fprintf(outf,"\n");
        is=is+7;
      }
      else if(read_array[is] == 0xe0000000) {
       iset = 0;
       for(i=0; i< 2*nword; i++) {
        if((read_array_s[i] & 0xf000) == 0x4000) {
         iset=1;
         ncount=0;
         if(iwrite !=1 ) printf(" channel %d\n",(read_array_s[i] & 0xfff));
         else fprintf(outf, " channel %d\n",(read_array_s[i] & 0xfff));
        }
        else if ((read_array_s[i] & 0xf000) == 0x5000) {
          if(iwrite !=1 ) printf(" channel end %d\n",(read_array_s[i] &0xfff));
          else  fprintf(outf, " channel end %d\n",(read_array_s[i] &0xfff));
        }
        else if (iset ==1) {
         if(iwrite !=1 ) printf(" %4x",read_array_s[i]);
         else fprintf(outf," %4x",read_array_s[i]);
         ncount = ncount+1;
         if((ncount%8) == 0) {
          if(iwrite !=1 ) printf("\n");
          else fprintf(outf,"\n");
         ...
         if(iwrite !=1 ) printf(" %x",read_array_s[i]);
         else fprintf(outf," %x",read_array_s[i]);
         ncount = ncount+1;
         if((ncount%8) == 0) {
          if(iwrite !=1 ) printf("\n");
          else fprintf(outf,"\n");
       ...
       printf(" end of event, type 1 to continue \n");
       scanf("%d",&ik);
       is=is+1;
       nword =0;
       ia=0;
        ...
        read_array_s[ia] = read_array[is] & 0xffff;
        read_array_s[ia+1] = (read_array[is] >> 16) & 0xffff;
        ia = ia+2;
        nword = nword+1;
        is=is+1;
      }
...
//      k = (j+1) + (j<<16);
      ...
      buf_send[i] = k ;
      j=j+2;
//      printf(" i= %d, buf_send[i] = %8x\n",i, buf_send[i]);
     }
     buf_send[0]=0x05550aaa;
     buf_send[1]=0x05550aaa;
     buf_send[2]=0x05550aaa;
     i=1;
     k=1024;
     i = pcie_send_1(hDev3, i, k, px);
//
//
//
     i = pcie_send_1(hDev3, i, k, px);
...
//
     printf(" boot fpga and send data finished, and wait \n");
     scanf("%d", &izero);
//
//
//
...
//
//
//enable the run command   --- from the controller
     imod=0;
     ichip=1;
     buf_send[0]=(imod<<11)+(ichip<<8)+(mb_cntrl_set_run_on)+(0x0<<16); //enable offline run off
     i=1;
     k=1;
     i = pcie_send(hDev, i, k, px);
//
     printf(" run on and wait \n");
     scanf("%d", &izero);
//
     timesize =500;
     iframe_length =4095;
...
#define mb_trig_rd_status 32
  ...
  if(hDevPMT){
    ...
    imod=imod_trig;
    ichip=0;   // don't care about chip number for triggr module
    //    buf_send[0]=(imod<<11)+(ichip<<8)+mb_trig_rd_counter+(0x0<<16);  // read out status // JICA: COMMENTED BECAUSE mb_trig_rd_counter is not defined
    i=1;
    k=1;
    i = pcie_send(hDevPMT, i, k, px);
    usleep(10);
  ...
  if( hDevPMT ){ // Jose: need this to isolate hDevPMT-needed lines 
    // GPS
    imod=imod_trig;
