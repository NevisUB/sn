...
static int baselineValue; // fix baseline
...
int main(int argc, char **argv)
{
  //  if( argc != 3 ){
  if( argc != 5 ){
    printf("Run it as ./mbtest_trial_case1_fixBaseline $amplitudeThreshold $baselineMean $baselineVariance $baselineValue. Exit... \n");
    exit(-1);
  }
...
//
       ...
       sprintf(nameSetup, "%4i%02i%02i%02i%02i%02i_threshold%i_Bmean%i_Bvar%i_fixB%i_bipolar2",
	       ltm.tm_year + 1900, ltm.tm_mon + 1, ltm.tm_mday, ltm.tm_hour, ltm.tm_min, ltm.tm_sec,
	       aThresh, bMean, bVar, baselineValue
	       );
       ...
       for (is=0; is<64; is++) {
        ...
        ibase = 2040;    // approximate baseline for empty channel
...
//         else ijk=ibase;
	  ...
	  // *** Signal ***
	  // Different shape for even and odd channels
	  //if( is & 0b1 ){ // Odd channels
	    //if( ik > 250 && ik < 256) ijk -= 300; 
	    //if( ik < 2 ) ijk += 20;
	    //if( ik > 240 && ik < 246) ijk -= 300; 
	  //} else { // Even channels
	    //if( ik < 2 ) ijk += 20;
	    //if( ik > 240 && ik <= 250) ijk -= 300; // bipolar
	    //if( ik > 250 && ik < 256) ijk += 300; 
	    //if( ik > 230 && ik <= 240) ijk -= 300; // bipolar
	    //if( ik > 240 && ik < 246) ijk += 300; 
	  //}

	  // *** Huffman-incompressible sawtooth *** 
	  ijk = ibase;
	  // Avoid rollover to simplify
	  if( ik > 9 && ik < 246 ) ijk += 4*(ik - 127);
